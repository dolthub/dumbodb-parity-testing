...
func TestDB_RunCommand_ServerStatus(t *testing.T) {
	harness.PairTest(t, harness.TestCase{
		...
		Run: func(ctx context.Context, col *mongo.Collection) (interface{}, error) {
			...
			// repl, sharding, storageEngine, tcmalloc, version (docudolt reports a stub version),
			// host (differs between Docker/native deployments),
			// uptime/connections/opcounters (vary or absent in docudolt).
...
func TestDB_RunCommand_CollStats(t *testing.T) {
	harness.PairTest(t, harness.TestCase{
		...
		Run: func(ctx context.Context, col *mongo.Collection) (interface{}, error) {
			...
			// size/avgObjSize/storageSize/totalSize (docudolt computes these differently from WiredTiger).
