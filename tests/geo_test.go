// Parity tests migrated from dolthub/dumbodb/tests/geo_test.go.
// All geo tests have been graduated to DumboDBFull — geospatial support is implemented in DumboDB.
package tests
...
func TestGeo_near_InvalidPointLongitude(t *testing.T) {
	harness.PairTest(t, harness.TestCase{
		...
		Support: harness.DumboDBXFail, // Diverge (do-bpmo): dumbodb omits "invalid point in geo near query $geometry argument: {...}" prefix; fix exists in WIP local dumbodb, not yet on GitHub main
		Setup:   geo2dSphereSetup("loc"),
		Run: func(ctx context.Context, col *mongo.Collection) (interface{}, error) {
			filter := bson.D{{Key: "loc", Value: bson.D{{Key: "$near", Value: bson.D{
...
func TestGeo_geoNear_InvalidPoint(t *testing.T) {
	harness.PairTest(t, harness.TestCase{
		...
		Support: harness.DumboDBXFail, // Diverge (do-bpmo): dumbodb reports lng/lat out-of-bounds instead of "invalid argument in geo near query: type"; fix exists in WIP local dumbodb, not yet on GitHub main
		Setup:   geo2dSphereSetup("loc"),
		Run: func(ctx context.Context, col *mongo.Collection) (interface{}, error) {
			pipeline := bson.A{bson.D{{Key: "$geoNear", Value: bson.D{
