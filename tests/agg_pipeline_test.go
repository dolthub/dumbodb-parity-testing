...
// MongoDB returns error code 40198; dumbodb returns 9. Bug: do-to0, do-slq1.
func TestAgg_bucket_missingBoundaries(t *testing.T) {
	harness.PairTest(t, harness.TestCase{
		...
		Run: func(ctx context.Context, col *mongo.Collection) (interface{}, error) {
			...
			// but MongoDB returns code 40198 while dumbodb returns code 9.
			_, err := runPipeline(ctx, col, []bson.D{
				{{Key: "$bucket", Value: bson.D{
					{Key: "groupBy", Value: "$price"},
				}}},
