// Parity tests migrated from dolthub/dumbodb/tests/agg_stages_test.go.
...
func TestAggStage_limit_LimitZeroError(t *testing.T) {
	harness.PairTest(t, harness.TestCase{
		...
		Support: harness.DumboDBXFail, // error code mismatch: mongo=15958, dumbodb=5107201
		Run: func(ctx context.Context, col *mongo.Collection) (interface{}, error) {
			_, err := col.Aggregate(ctx, bson.A{
...
func TestAggStage_bucket_MissingBoundariesError(t *testing.T) {
	...
	// error code 40198; dumbodb returns code 9 (FailedToParse). Fix in dumbodb's
...
func TestAggStage_bucketAuto_MissingBucketsError(t *testing.T) {
	...
	// error code 40246; dumbodb returns code 9 (FailedToParse). Fix in dumbodb's
...
func TestAggStage_unsupportedErrors_densify(t *testing.T) {
	harness.PairTest(t, harness.TestCase{
		...
		Support: harness.DumboDBXFail, // error code diverges: mongo=IDLFailedToParse, dumbodb=Location40414
		Run: func(ctx context.Context, col *mongo.Collection) (interface{}, error) {
...
func TestAggStage_unknownStageError(t *testing.T) {
	...
	// 40324 (Location40324); dumbodb returns 40234 (Location40234). A separate
